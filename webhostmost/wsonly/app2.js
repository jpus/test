...
const originalUUID = process.env.UUID || 'ffffffff-ffff-ffff-ffff-ffffffffffff'; // 配置 UUID
const uuid = originalUUID.replace(/-/g, ''); // 无须理会
const DOMAIN = process.env.DOMAIN || 's3whm.去你大爷.cnm';  //项目域名或已反代的域名，不带前缀，建议填已反代的域名
